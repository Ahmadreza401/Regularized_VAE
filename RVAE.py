...
experiment_id = "plots/VRAE/VRAE-3"
...
# inputs_train, inputs_validation, inputs_test = split(samples, [0.6, 0.2, 0.2])

save_plot_sample(samples[-6:], '', '00-real_data', path=experiment_id)
...
for config in configs:
    ...
    config_id = str(config).replace(", ", "_").replace("'", "")[1:-1]
    # if verbose > 0:
        # print(config_id)
    ...
    while keep_training:
        ...
        for mbi in range(num_mini_batches_train):
            ...
            if len(input_) == batch_size:
                ...
                # weighted mean
                weighted_cost_sum += train_res[1] * len(input_)
        cost_train = weighted_cost_sum / len(inputs_train)
        costs_train.append(cost_train)
        # if verbose > 1:
        #     print(train_res[2])
        ...
        if (epoch_counter % 10) == 0:
            save_plot_sample(test_res[1][-6:], "--epoch" + str(epoch_counter).zfill(4), str(experiment_counter)
                             + '-' + 'E,'+ str(hidden_units_enc) + '-D,' + str(hidden_units_dec) + '-dim_z,' +
                             str(emb_dim)+'-mult,' + str(mult) + '-LR,' + str(learning_rate)
                             , path=experiment_id)
    ...
    # plt.show()
    plt.savefig("./" + experiment_id + "/" + str(experiment_counter) + '-' + config_id + "_learning_curves.png", dpi=300)

    save_plot_sample(test_res[1][-6:], "--epoch" + str(epoch_counter).zfill(4),str(experiment_counter)
                             + '-' +'E,'+ str(hidden_units_enc) + '-D,' + str(hidden_units_dec) + '-dim_z,' +
                             str(emb_dim)+'-mult,' + str(mult) + '-LR,' + str(learning_rate)
                             , path=experiment_id)
    print('Did not learn more after epoch' + str(epoch_counter))
    ...
    to_store = {'config': config, 'costs_train': costs_train, 'costs_val': costs_val, 'costs_test': costs_test,
                ...
                'other_scores_validation': other_scores_validation,
                'other_scores_test': other_scores_test}

    with open("./" + experiment_id + "/" + str(experiment_counter) + '-' + config_id + ".json", "w") as f:
