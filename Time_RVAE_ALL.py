...
class TimeVAE:
    def make_encoder(self, config, data, code_size):
        ...
        cell = tf.contrib.rnn.LSTMCell(num_units=100, state_is_tuple=True)
        ...
        self.z_mean = loc = tf.layers.dense(enc_rnn_states[1], code_size)
        self.z_log_sigma_sq = scale = tf.layers.dense(enc_rnn_states[1], code_size, tf.nn.softplus)
    ...
    def make_decoder(self, config, code, data_shape):
        ...
        # code = tf.reshape(code, shape=[-1, 5])
        dec_initial_state = tf.layers.dense(code, 100, activation=tf.nn.tanh)
        # dec_inputs = tf.zeros(shape=(tf.shape(code)[0], 30, 1))
        dec_inputs = tf.zeros(shape=([tf.shape(code)[0]] + self.D))

        w_out_dec = tf.get_variable(name='w', shape=[100, 1], dtype=tf.float32
                                    , initializer=tf.truncated_normal_initializer)
        # w_out_dec = tf.variable( tf.truncated_normal([50, 1]))

        b_out_dec = tf.get_variable(name='b', shape=[1], dtype=tf.float32,
                                    initializer=tf.truncated_normal_initializer)
        # b_out_dec = tf.variable(tf.truncated_normal([1]))
        ...
        dec_rnn_outputs, dec_rnn_states = tf.nn.dynamic_rnn(
            ...
            inputs=dec_inputs)
        rnn_outputs_2d = tf.reshape(dec_rnn_outputs, [-1, 100])
        logits_2d = tf.matmul(rnn_outputs_2d, w_out_dec) + b_out_dec
        self.gen_output = logit = tf.reshape(logits_2d, [-1] + data_shape)
        return tfp.distributions.Independent(tfp.distributions.Bernoulli(logit))

    def __init__(self, D, dim_z, beta, mult, config=None):
        ...
        self.latent_loss = self.mult * (-0.5) * tf.reduce_sum(1 + self.z_log_sigma_sq
    ...
    def fit(self, X, input_type, exp_number, path_exp, path_save, epochs= 500, batch_sz=100):
        ...
        while keep_training:
            ...
            if epoch_counter % epoch_falg == 0 and epoch_counter != 0:
                sd.save_plot_sample(last_test_gen[:6], "--epoch" + str(epoch_counter).zfill(4),
                                    ...
                                    + '-beta,' + str(self.beta) + '-mult,' + str(self.mult), path=path_experiment)
        ...
        with open(path_experiment + '/' + str(experiment_number).zfill(4) + '-' + str(self.config) + '-' + '-dim_z,' + str(self.dim_z)
                    + '-beta,' + str(self.beta) + '-mult,' + str(self.mult) + '-Saved_costs' + ".json",
                  "w") as f:
            json.dump(to_store, f)

        self.saver.save(self.sess, path_save + '/'+str(experiment_number).zfill(4) + '-' + str(self.config) + '-' + '-dim_z,'
                        + str(self.dim_z) + '-beta,' + str(self.beta) + '-mult,' + str(self.mult))
